...
其次，写信息的代码被移动到了一个独立的函数中②。因为每个线程都必须具有一个初始函数(*initial function*)，新线程的执行在这里开始。对于应用程序来说，初始线程是 main()，但是对于其他线程，可以在`std::thread`对象的构造函数中指定——在本例中，被命名为**t**③的`std::thread`对象拥有新函数**hello()**作为其初始函数。
